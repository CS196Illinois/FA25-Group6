...
func _ready():
	...
	# Only start dialog if this GameManager is in the game scene (not the autoload)
	# Check if we have a ScoreLabel child (only the scene instance has this)
	if has_node("ScoreLabel"):
		score_label = $ScoreLabel
		#start beginning dialog when game actually starts
