...
func _physics_process(delta: float) -> void:
	# Talk to NPC with F key (separate from beat)
	# Only start dialogue if not already in one
	if Input.is_action_just_pressed("scare") and player_enter and !has_talked and !GameManager.is_in_dialogue:
		has_talked = true
		is_talking = true
		GameManager.is_in_dialogue = true  # Mark dialogue as active
	...
	# Original beat/scare mechanic on "interact" button
...
func _on_dialogue_ended():
	...
	GameManager.is_in_dialogue = false  # Mark dialogue as inactive
